    ...
    def __init__(self, num_classes=6, input_size=768 * 2,
        ...
        for i in range(num_layers):
            if i == 0:
                ...
                ## why input size == hidden_size == 768????????
...
#             print(sequence_output[0].size())
#             print(sequence_output[1].size())
            hidden = torch.cat(hidden, dim = 1)
            
        print('hidden size', hidden.size())
        log_prob = self.discriminator(hidden) ############# eerror place ## eerror place ## eerror place ## eerror place 
